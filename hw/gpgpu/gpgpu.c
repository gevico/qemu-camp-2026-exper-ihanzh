...
static uint64_t gpgpu_ctrl_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    case GPGPU_REG_DMA_SIZE:
        return s->dma.size & 0xffffffff;
    case GPGPU_REG_DMA_CTRL:
        return s->dma.ctrl & 0b111;
    case GPGPU_REG_DMA_STATUS:
        return s->dma.status & 0b111;
                             ...
                             unsigned size)
{
    ...
    case GPGPU_REG_DMA_SRC_HI:
        s->dma.src_addr =
            (s->dma.src_addr & 0xffffffffULL) | ((val & 0xffffffff) << 32);
        break;
    case GPGPU_REG_DMA_DST_LO:
        s->dma.dst_addr =
            (s->dma.dst_addr & ~(0xffffffffULL)) | (val & 0xffffffff);
        break;
    case GPGPU_REG_DMA_DST_HI:
        s->dma.dst_addr =
            (s->dma.dst_addr & 0xffffffffULL) | ((val & 0xffffffff) << 32);
        break;
    case GPGPU_REG_DMA_SIZE:
        s->dma.size = val & 0xffffffff;
        break;
    case GPGPU_REG_DMA_CTRL:
        s->dma.ctrl = (val & 0b111);
        break;
    case GPGPU_REG_THREAD_ID_X:
